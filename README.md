...
- 선택적 MC 초기화 렌더 및 manifests 포함 지원
...
`mc_init`은 선택적으로 렌더하고, 선택적으로 manifests에 포함할 수 있습니다.
...
- `MC_INIT_ENABLE`
  - `yes`면 렌더 수행
  - `no`면 전체 skip
- `MC_INIT_COPY_TO_MANIFESTS`
  - `yes`면 렌더된 MC를 `${INSTALL_WORKDIR}/openshift`로 복사
  - `no`면 렌더만 수행

## 출력 경로

- `MC_INIT_RENDER_DIR`
  - 기본값: `${INSTALL_WORKDIR}/mc_init_rendered`
- `MC_INIT_MANIFESTS_TARGET_DIR`
  - 기본값: `${INSTALL_WORKDIR}/openshift`

## 선택적 MC on/off

- `MC_ENABLE_CHRONY`
- `MC_ENABLE_REGISTRIES`
- `MC_ENABLE_CORE_PASSWORD`
- `MC_ENABLE_ROOT_PASSWORD`
- `MC_ENABLE_THP`
...
- `MC_THP_HUGEPAGESZ`
- `MC_THP_HUGEPAGES`
- `MC_THP_DISABLE_TRANSPARENT_HUGEPAGE`
  - `yes`면 `transparent_hugepage=never` 추가
  - `no`면 제외
...
## 02-install/01a-mc-init-render.sh
역할:
- 선택된 `mc_init` 항목 렌더
- 렌더 결과를 `${INSTALL_WORKDIR}/mc_init_rendered`에 저장
- 필요 시 `${INSTALL_WORKDIR}/openshift`로 복사
...
#### `MC_INIT_ENABLE=yes`, `MC_INIT_COPY_TO_MANIFESTS=no`
- 렌더만 수행
- `${INSTALL_WORKDIR}/mc_init_rendered`에 파일 남김

#### `MC_INIT_ENABLE=yes`, `MC_INIT_COPY_TO_MANIFESTS=yes`
- 렌더 수행
- 렌더 결과를 `${INSTALL_WORKDIR}/openshift`로 복사

### 세부 분기

#### chrony
- bastion 1개 → chrony server = `PXE_BASTION_IP`
- bastion 2개 이상 → chrony server = `SERVICE_VIP`

#### registries
- mirror registry host = `${HOST}.${CLUSTER_NAME}.${BASE_DOMAIN}`

#### core/root password
- 평문 입력
- 스크립트가 내부에서 해시 및 base64 처리

#### THP
- `isolcpus`, `hugepagesz`, `hugepages`는 변수값 사용
- `transparent_hugepage=never`는 on/off로만 결정
...
- mc_init chrony server = bastion 실제 IP

## bastion 수 >= 2
- keepalived enable 가능
- PXE nameserver = DNS_SERVER
- DNS api/api-int/apps = VIP
- mc_init chrony server = VIP
...
## MC_INIT_COPY_TO_MANIFESTS=yes
- 렌더된 MC를 manifests에 포함
